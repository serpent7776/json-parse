...
{
	// return Ok{std::forward<T>(val), s}; // maybe used uninitialised here warning
	return Ok{val, s};
}
template<typename T>
Result<T> ok(const T& val, std::string_view s)
{
	return Ok{val, s};
}
template<typename T>
...
{
	return JsonPart{ok(Json{std::forward<T>(val)}, s)};
}
...
auto ask(std::string_view s, F&& f) -> Result<std::string>
{
	...
	{
		const auto r = f(span);
		if (auto err = std::holds_alternative<Err>(r))
		{
...
{
	return match(take(s, is_digit),
		[](const std::string& int_str, std::string_view s) {
			const long integer = stol(int_str);
			const auto r = match(chr(s, '.'),
